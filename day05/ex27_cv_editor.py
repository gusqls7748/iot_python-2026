...
# 5. 편집 이미지 저장
edit_img = None    # 전역변수로 사용하겠다는 선언



# 이미지로드
img = cv2.imread(filename='./day05/cat.jpg')
if img is None:
...
def update(_=None):
    ...
    blur = cv2.getTrackbarPos('Blur', winname) # 트랙바 이름 Blur에서 가져온다
...
cv2.createTrackbar('Edge', winname, 0, 200, update) # 이미지 블러 변경
cv2.createTrackbar('Rotate', winname, 180, 360, update) # 이미지 블러 변경
